...
def find_매수신호(df_ohlcv, li_지지저항, dt_일자시간=None):
    ...
    # 몸통이 양봉
    b_매수신호1_거래량_양봉 = (n_종가_1 / n_시가_1 - 1) * 100 > 0
    # 몸통이 +2% 이내
    b_매수신호1_거래량_몸통 = (n_종가_1 / n_시가_1 - 1) * 100 < 2
    # 거래량 조건
    b_매수신호1_거래량 = b_매수신호1_거래량_z값 and b_매수신호1_거래량_양봉 and b_매수신호1_거래량_몸통

    # 이평선 터치 (0.3% 이내)
    b_매수신호1_이평선_ma10 = abs(n_저가_1 / n_ma10_1 - 1) * 100 < 0.3
    b_매수신호1_이평선_ma20 = abs(n_저가_1 / n_ma20_1 - 1) * 100 < 0.3
    b_매수신호1_이평선_ma60 = abs(n_저가_1 / n_ma60_1 - 1) * 100 < 0.3
    b_매수신호1_이평선_터치 = b_매수신호1_이평선_ma10 or b_매수신호1_이평선_ma20 or b_매수신호1_이평선_ma60
    # 몸통이 +1% 초과
    b_매수신호1_이평선_몸통 = (n_종가_1 / n_시가_1 - 1) * 100 > 1
    # 이평선 조건
    b_매수신호1_이평선 = b_매수신호1_이평선_터치 and b_매수신호1_이평선_몸통
    ...
    b_매수신호2 = b_매수신호2_종가 and b_매수신호2_ma20 and b_매수신호2_ma60 and b_매수신호2_ma120
    ...
    # 4) [sr검증] 지지저항 위치
    # 종가가 지지저항 내부에 존재
    b_매수신호4_sr내부 = max(li_지지저항) > n_종가_1 > min(li_지지저항)
    # 저항선 대비 1.5% 이상 갭 존재
    n_저항선 = min(저항 for 저항 in li_지지저항 if 저항 > n_종가_1) if b_매수신호4_sr내부 else None
    b_매수신호4_저항마진 = (n_저항선 / n_종가_1 - 1) * 100 > 1.5 if n_저항선 is not None else False
...
def find_매도신호(dic_기준정보, n_현재가=None):
    ...
    b_매도신호3 = b_매도신호3_종가 or b_매도신호3_ma20 or b_매도신호3_ma60 or b_매도신호3_ma120
