...
def trd_make_추가데이터_종목모델_rf(df):
    ...
    # 과거 데이터 추가
    li_대상컬럼 = [컬럼명 for 컬럼명 in df_추가.columns if 컬럼명 not in ['일자', '종목코드', '종목명', '시간']]
    for n in range(3):
        n_이전봉 = n + 1
    ...
    # 시간 구간 생성
    df_추가 = pd.concat([df_추가, (df_추가['시간'] >= '09:00:00') & (df_추가['시간'] < '10:30:00')], axis=1)
    df_추가.columns = list(df_추가.columns)[:-1] + ['타임존_A']
    df_추가 = pd.concat([df_추가, (df_추가['시간'] >= '10:30:00') & (df_추가['시간'] < '11:30:00')], axis=1)
    df_추가.columns = list(df_추가.columns)[:-1] + ['타임존_B']
    df_추가 = pd.concat([df_추가, (df_추가['시간'] >= '11:30:00') & (df_추가['시간'] < '13:00:00')], axis=1)
    df_추가.columns = list(df_추가.columns)[:-1] + ['타임존_C']
    df_추가 = pd.concat([df_추가, (df_추가['시간'] >= '13:00:00') & (df_추가['시간'] < '14:30:00')], axis=1)
    df_추가.columns = list(df_추가.columns)[:-1] + ['타임존_D']
    df_추가 = pd.concat([df_추가, (df_추가['시간'] >= '14:30:00') & (df_추가['시간'] < '15:40:00')], axis=1)
    df_추가.columns = list(df_추가.columns)[:-1] + ['타임존_E']
