...
def make_추가데이터_rf(df):
    ...
    n_봉수 = 3
    df_추가['임시_고가_max'] = df_추가['고가'].rolling(n_봉수).max().shift(-1 * (n_봉수 - 1))
    df_추가['임시_저가_min'] = df_추가['저가'].rolling(n_봉수).min().shift(-1 * (n_봉수 - 1))

    df_추가['임시_상승률(%)_고가_max'] = (df_추가['임시_고가_max'] / df_추가['종가_1봉'] - 1) * 100
