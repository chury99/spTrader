...
# def cal_z스코어_old(데이터, b_매도검증=False):
#     """ 입력 받은 데이터 기준으로 마지막 값의 z스코어 계산 후 리턴 """
#     # nan 제거 (nan : 자기 자신과 같지 않음)
#     ary_데이터 = np.array([x for x in 데이터 if x == x])
#     if len(ary_데이터) < 20 and not b_매도검증:
#         return np.nan
#     if len(ary_데이터) < 2 and b_매도검증:
#         return np.nan
#
#     # 데이터 계산
#     n_평균값 = ary_데이터.mean()
...
def make_매수신호(dic_매개변수, dt_일자시간=None):
    ...
    if len(df_초봉) == 0:
        ...
        #                 n_매수횟수_매수봇='', n_매도횟수_매수봇='', n_체결횟수_0카운트='',
    ...
    # ary_체결횟수 = df_초봉['체결횟수'].values[-30:]
    n_매수량 = df_초봉['매수량'].values[-1]
    n_매도량 = df_초봉['매도량'].values[-1]
    n_체결수횟수 = df_초봉['체결횟수'].values[-1]
...
def make_매도신호(dic_매개변수, dt_일자시간=None):
    ...
    if len(df_초봉) == 0:
        ...
        #                 n_매수량max_매도봇='', n_매수량_매도봇='', n_z매수량_매도봇='',
    ...
    #                 n_경과초_매도봇=n_경과초,
    #                 n_매도량_누적_매도봇=n_매도량_누적, n_매수량_누적_매도봇=n_매수량_누적, n_z매수량_매도봇=n_z매수량,
    #                 n_매수량max_매도봇=n_매수량max, n_매수량_기준봉_매도봇=n_매수량_기준봉, n_매수량_매도봇=n_매수량,
    #                 li_신호종류_매도봇=li_신호종류, li_매도신호_수치_매도봇=li_매도신호_수치,
    #                 s_매도사유_매도봇=s_매도사유)
