...
class Analyzer:
    def __init__(self, b_멀티=False, s_시작일자=None, n_분석일수=None):
        ...
        self.folder_지표생성 = dic_폴더정보['tf종목분석|10_지표생성']
    ...
    def 분석_일봉변동(self, b_차트):
        ...
        # 분석대상 일자 선정
        li_일자_전체 = [re.findall(r'\d{8}', 파일명)[0] for 파일명 in os.listdir(self.folder_캐시변환)
        ...
        for s_일자 in li_일자_대상:
            ...
            for s_종목코드 in tqdm(dic_일봉_대상월[s_금월].keys(), desc=f'일봉 변동 분석|{s_일자}'):
                ...
                if b_차트 and len(df_일봉변동_종목) > 0:
                    ...
                    # 차트 저장
                    folder_그래프 = os.path.join(f'{self.folder_일봉변동}_그래프', f'일봉변동_{s_일자}')
            ...
            # log 기록
            self.make_log(f'일봉변동 분석 완료({s_일자}, {len(df_일봉변동):,}종목)')
    ...
    def 분석_지표생성_종목별(self, s_종목코드):
        ...
        # csv 저장
        s_폴더 = os.path.join(f'{self.folder_지표생성}_종목별', f'지표생성_{s_일자}')
    ...
    def 분석_분봉확인(self, n_초봉):
        ...
        for s_일자 in li_일자_대상:
            ...
            for s_종목코드 in tqdm(dic_초봉.keys(), desc=f'분봉확인-{n_초봉}초봉-{s_일자}'):
                ...
                # csv 저장
                s_폴더 = os.path.join(f'{self.folder_분봉확인}_종목별', f'분봉확인_{s_일자}')
                os.makedirs(s_폴더, exist_ok=True)
