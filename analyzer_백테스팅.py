...
class Analyzer:
        ...
        for s_일자 in li_일자_대상:
            ...
            for s_시간 in tqdm(li_시간, desc=f'매수검증 ({s_일자})'):
                for s_종목코드 in dic_종목조건.keys():
                    ...
                    # 매수 신호 생성
                    df_검증['종목신호'] = (df_검증['종목확률(%)'] > n_확률스펙) * 1
    ...
    def 백테스팅_매도검증(self, s_모델):
        ...
        for s_일자 in li_일자_대상:
            ...
            # 매도 검증
            li_df_매도검증 = []
            for ary_매수 in tqdm(df_매수.values, desc=f'매도검증 ({s_일자})'):
                # 매수 정보 확인
                [s_일자, s_종목코드, s_종목명, s_시간, s_종목케이스, n_확률스펙종목, n_종목확률,
                 n_공통확률, n_종목신호, n_공통신호, n_매수신호] = ary_매수
                s_대기봉수, s_학습일수, s_rf_트리, s_rf_깊이 = s_종목케이스.split('_')
                n_대기봉수 = int(s_대기봉수)

                # 분봉 데이터 불러오기
                dt_시작 = pd.Timestamp(f'{s_일자} {s_시간}')
                dt_종료 = dt_시작 + pd.Timedelta(minutes=(n_대기봉수 * 10))
                df_분봉 = dic_분봉[s_종목코드]
                df_분봉 = df_분봉[df_분봉.index >= dt_시작]
                df_분봉 = df_분봉[df_분봉.index <= dt_종료]
                df_분봉 = df_분봉.sort_index(ascending=True)
                if len(df_분봉) == 0:
                    continue

                # 10분봉 데이터 불러오기 (tr 데이터 동일 형태)
                df_10분봉 = dic_10분봉[s_종목코드]
                df_10분봉_tr = df_10분봉[df_10분봉.index <= dt_시작].sort_index(ascending=False)

                # 매수 정보 산출 (분봉 기준 매수 시점 데이터)
                s_시간_매수 = df_분봉['시간'].values[0]
                n_단가_매수 = int(df_분봉['고가'].values[0])

                # 기준 단가 불러오기 불러오기 (10분봉 기준 이전 분봉 종가)
                n_단가_기준 = int(df_10분봉_tr['종가'].values[1])

                # 매도 검증 (분봉)
                s_시간_매도, n_단가_매도, s_매도이유 = None, None, None
                for ary_분봉 in df_분봉.values:
                    # 데이터 확인
                    [s_일자, s_종목코드, s_종목명, s_시간, n_시가, n_고가, n_저가, n_종가, n_거래량, n_전일종가, n_전일대비,
                     n_종가ma5, n_종가ma10, n_종가ma20, n_종가ma60, n_종가ma120,
                     n_거래량ma5, n_거래량ma20, n_거래량ma60, n_거래량ma120] = ary_분봉
                ...
                # 동시호가 확인
                df_10분봉_동시호가 = df_10분봉_tr[df_10분봉_tr['시간'] != '15:30:00']
                ary_10분봉_동시호가 = df_10분봉_동시호가.loc[:, '시가': '종가'].values
                li_동시호가 = [all(x == ary[0] for x in ary) for ary in ary_10분봉_동시호가]
                if li_동시호가[0] is True:
                    s_시간_매수 = df_10분봉_tr['시간'].values[0]
                    s_시간_매도 = s_시간_매수
                    n_단가_매도 = n_단가_매수
                    s_매도이유 = '동시호가'
                ...
                li_매도검증 = [s_일자, s_종목코드, s_종목명, s_대기봉수, n_종목신호, n_공통신호, n_매수신호, n_단가_기준,
                           s_시간_매수, n_단가_매수, s_시간_매도, n_단가_매도, s_매도이유]
                li_컬럼명 = ['일자', '종목코드', '종목명', '대기봉수', '종목신호', '공통신호', '매수신호', '단가_기준',
                          '시간_매수', '단가_매수', '시간_매도', '단가_매도', '매도이유']
                df_매도검증_시간 = pd.DataFrame([li_매도검증], columns=li_컬럼명)
    ...
    def 백테스팅_수익검증(self, s_모델, b_카톡=False):
        ...
        for s_일자 in li_일자_대상:
            ...
            df_수익검증_전체 = pd.concat(li_df_수익검증, axis=0)

            # 수익률 생성 (세금+수수료 0.3% 반영)
            df_수익검증_전체['수익률(%)'] = (df_수익검증_전체['단가_매도'] / df_수익검증_전체['단가_매수'] - 1) * 100 - 0.3

            # 매수신호 골라내기 (종목신호 & 공통신호)
            df_수익검증 = df_수익검증_전체[df_수익검증_전체['매수신호'] == 1].copy()
            ...
            li_df_수익검증 = list()
            for s_그룹키 in df_수익검증_전체['일자'].unique():
                try:
                    df_수익검증_개별 = gr_df_수익검증.get_group(s_그룹키).copy()
                    ary_직전매도시간 = df_수익검증_개별['시간_매도'].shift(1, fill_value='00:00:00').values
                    df_수익검증_개별['시간확인'] = (df_수익검증_개별['시간_매수'].values > ary_직전매도시간) * 1
                    df_수익검증_신호처리 = df_수익검증_개별[df_수익검증_개별['시간확인'] == 1].iloc[:, : -1]
                    li_df_수익검증.append(df_수익검증_신호처리)
                except KeyError:
                    if len(li_df_수익검증) == 0:
                        continue
                    else:
                        df_수익검증_신호처리 = li_df_수익검증[-1].copy()
                        df_수익검증_신호처리 = df_수익검증_신호처리.drop(df_수익검증_신호처리.index)
                        df_수익검증_신호처리.loc[0] = None
                        df_수익검증_신호처리['일자'] = s_그룹키
    ...
    @staticmethod
    def make_리포트_백테스팅(df_감시대상, df_수익검증_분석_종목, df_수익검증_분석_공통, df_수익검증_백테_종목, df_수익검증_백테_공통):
        ...
        df_월별테이블_T_분석_종목 = df_월별테이블_분석_종목[-1 * n_컬럼수_월별:].T
        ...
        df_일별테이블_T_분석_종목 = df_일별테이블_분석_종목[-1 * n_컬럼수_일별:].T
        ...
        df_월별테이블_T_분석_공통 = df_월별테이블_분석_공통[-1 * n_컬럼수_월별:].T
        ...
        df_일별테이블_T_분석_공통 = df_일별테이블_분석_공통[-1 * n_컬럼수_일별:].T
        df_일별테이블_T_분석_공통.index = ['월일', '예측(건)', '성공(건)', '성공률(%)', '기대수익(%)']

        # ### 수익검증 - 백테 - 종목 ### #
        # 데이터 추가 생성
        df_수익검증_백테_종목['상승예측'] = df_수익검증_백테_종목['종목신호'].apply(lambda x: 1 if x == 1 else 0)
        df_수익검증_백테_종목['예측성공'] = df_수익검증_백테_종목['수익률(%)'].apply(lambda x: 1 if x > 0 else 0)
        df_수익검증_백테_종목['년월'] = df_수익검증_백테_종목['일자'].apply(lambda x: f'{x[2:4]}-{x[4:6]}')
        sri_년월 = df_수익검증_백테_종목.groupby('년월')['년월'].first()
        df_수익검증_백테_종목['년월일'] = df_수익검증_백테_종목['일자']
        sri_년월일 = df_수익검증_백테_종목.groupby('년월일')['년월일'].first()
        # 월별 데이터 생성
        li_월별테이블_백테_종목 = list()
        ...
        df_월별테이블_T_백테_종목 = df_월별테이블_백테_종목[-1 * n_컬럼수_월별:].T
        ...
        df_일별테이블_T_백테_종목 = df_일별테이블_백테_종목[-1 * n_컬럼수_일별:].T
        df_일별테이블_T_백테_종목.index = ['월일', '예측(건)', '성공(건)', '성공률(%)', '기대수익(%)']

        # ### 수익검증 - 백테 - 공통 ### #
        # 데이터 추가 생성
        df_수익검증_백테_공통['상승예측'] = df_수익검증_백테_공통['매수신호'].apply(lambda x: 1 if x == 1 else 0)
        df_수익검증_백테_공통['예측성공'] = df_수익검증_백테_공통['수익률(%)'].apply(lambda x: 1 if x > 0 else 0)
        df_수익검증_백테_공통['년월'] = df_수익검증_백테_공통['일자'].apply(lambda x: f'{x[2:4]}-{x[4:6]}')
        sri_년월 = df_수익검증_백테_공통.groupby('년월')['년월'].first()
        df_수익검증_백테_공통['년월일'] = df_수익검증_백테_공통['일자']
        sri_년월일 = df_수익검증_백테_공통.groupby('년월일')['년월일'].first()
        # 월별 데이터 생성
        li_월별테이블_백테_공통 = list()
        ...
        df_월별테이블_T_백테_공통 = df_월별테이블_백테_공통[-1 * n_컬럼수_월별:].T
        ...
        df_일별테이블_T_백테_공통 = df_일별테이블_백테_공통[-1 * n_컬럼수_일별:].T
