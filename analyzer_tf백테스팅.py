    ...
    def 검증_결과요약(self, b_차트, n_초봉):
        ...
        for s_일자 in li_일자_대상:
            ...
            for s_파일일자 in li_파일일자:
                ...
                df_결과_Tout = df_결과[df_결과['타임아웃']]
                df_결과_Tin = df_결과[df_결과['타임아웃'] == False]
                ...
                df_일별요약['Tout|전체거래'] = int(len(df_결과_Tout))
                df_일별요약['Tout|수익거래'] = int(len(df_결과_Tout[df_결과_Tout['수익률%'] > 0]))
                df_일별요약['Tout|성공률%'] = (df_일별요약['Tout|수익거래'] / df_일별요약['Tout|전체거래']) * 100
                df_일별요약['Tout|수익률%'] = df_결과_Tout['수익률%'].sum()
                df_일별요약['Tout|평균수익률%'] = df_일별요약['Tout|수익률%'] / df_일별요약['Tout|전체거래']
                df_일별요약['Tout|평균보유초'] = df_결과_Tout['보유초'].mean()
                df_일별요약['Tout|종목수'] = len(df_결과_Tout['종목코드'].unique())
                df_일별요약['Tout|종목당거래'] = df_일별요약['Tout|전체거래'] / df_일별요약['Tout|종목수']
                df_일별요약['Tin|전체거래'] = int(len(df_결과_Tin))
                df_일별요약['Tin|수익거래'] = int(len(df_결과_Tin[df_결과_Tin['수익률%'] > 0]))
                df_일별요약['Tin|성공률%'] = (df_일별요약['Tin|수익거래'] / df_일별요약['Tin|전체거래']) * 100
                df_일별요약['Tin|수익률%'] = df_결과_Tin['수익률%'].sum()
                df_일별요약['Tin|평균수익률%'] = df_일별요약['Tin|수익률%'] / df_일별요약['Tin|전체거래']
                df_일별요약['Tin|평균보유초'] = df_결과_Tin['보유초'].mean()
                df_일별요약['Tin|종목수'] = len(df_결과_Tin['종목코드'].unique())
                df_일별요약['Tin|종목당거래'] = df_일별요약['Tin|전체거래'] / df_일별요약['Tin|종목수']
    ...
    def 검증_수익요약(self, b_차트):
        ...
        for s_일자 in li_일자_대상:
            ...
            for s_초봉 in li_초봉:
                ...
                df_수익요약[s_초봉] = [df_결과['수익률%'].sum()] + list(df_결과['수익률%'])
